...
function setup() {
  ...
  mycanvas = createCanvas(windowWidth, windowHeight-50);
  slider = new Slider();
 slider.s();
      slider.u();
 ...
 for(let it = 0; it < iterations; it++){
  for (var x = 1; x < resX - 1; x++) {
    for (var y = 1; y < resY - 1; y++) {
      ...
      let grow=feed+(noise(x*0.1,y*0.1,x*y)*slider7.value()*0.1)
      let kill=k+(noise(x*0.1,y*0.1,x*y)*slider8.value()*0.1)
